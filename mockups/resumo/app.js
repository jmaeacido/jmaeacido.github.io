...
const isStaticMockup = location.pathname.includes("/mockups/resumo/") || location.hostname.endsWith("github.io");
...
async function analyze() {
  ...
  if (isStaticMockup && resumeFile.files[0] && !resumeText.value.trim()) {
    showAlert("Static preview mode", "GitHub Pages can score pasted resume text and TXT files. PDF and DOCX extraction still need the local PHP app.", "info");
    resumeText.focus();
    return;
  }
...
function buildStaticAnalysis() {
  ...
  const wordScore = Math.min(100, Math.max(35, Math.round(words.length / 4)));
  const sectionScore = Math.round(([hasContact, hasSummary, hasExperience, hasSkills, hasEducation].filter(Boolean).length / 5) * 100);
  const impactScore = /\b(increased|reduced|improved|managed|built|led|saved|automated|\d+%?)\b/i.test(text) ? 82 : 58;
  const jobTerms = currentMode === "job"
    ? Array.from(new Set((jobDescription.value.toLowerCase().match(/\b[a-z][a-z+#.-]{3,}\b/g) || [])
      .filter((term) => !["with", "from", "this", "that", "will", "your", "have", "work", "team"].includes(term))))
    : [];
  const resumeLower = text.toLowerCase();
  const missingKeywords = jobTerms.filter((term) => !resumeLower.includes(term)).slice(0, 10);
  const keywordScore = currentMode === "job" && jobTerms.length
    ? Math.round(((jobTerms.length - missingKeywords.length) / jobTerms.length) * 100)
    : 78;
  const overall = Math.round((wordScore * 0.25) + (sectionScore * 0.3) + (impactScore * 0.25) + (keywordScore * 0.2));

  return {
    mode: currentMode,
    title: currentMode === "job" ? "Static job match preview" : "Static resume score preview",
    subtitle: "GitHub Pages preview generated from browser-side checks.",
    engine: "Static mockup",
    ai_status: "Local PHP/Ollama analysis runs in the full Laragon app.",
    overall,
    resume_text: text,
    sections,
    metrics: [
      ["ATS Rating", `${overall}/100`, "Browser-side estimate"],
      ["Resume Status", overall >= 75 ? "Strong" : overall >= 60 ? "Developing" : "Needs work", "Static preview"],
      ["Detected Sections", String(sections.length), sections.length ? sections.map(toTitle).join(", ") : "No common sections found"]
    ],
    scores: {
      "Resume length": wordScore,
      "Section completeness": sectionScore,
      "Impact language": impactScore,
      "Keyword alignment": keywordScore
    },
    strengths: [
      hasContact ? "Contact details appear present." : "Resume text is ready for review.",
      hasExperience ? "Experience section is detectable." : "Core resume content can be scored from pasted text.",
      impactScore >= 80 ? "Impact-oriented wording or metrics were found." : "The preview identified areas where stronger impact wording could help."
    ],
    weaknesses: [
      hasSummary ? "Summary section detected." : "Add a short professional summary.",
      hasSkills ? "Skills section detected." : "Add a clear skills or tools section.",
      hasEducation ? "Education section detected." : "Add education, certifications, or training details where relevant."
    ],
    recommendations: [
      "Use measurable outcomes where possible.",
      "Keep section headings clear for ATS parsing.",
      "Run the local PHP version for PDF/DOCX extraction, database storage, and enhanced AI feedback."
    ],
...
function postAnalysis(formData) {
  if (isStaticMockup) {
    return new Promise((resolve) => {
      window.setTimeout(() => resolve({ analysis: buildStaticAnalysis() }), 650);
    });
  }
